...
namespace NeuralNetwork
{
	public class NeuralNet
	{
		...
		public NeuralNet DeepCopy()
		{
			NeuralNet net = new NeuralNet(InputLayer.Count, HiddenLayers[0].Count, OutputLayer.Count, HiddenLayers.Count, LearnRate, Momentum);
			for(int i = 0; i < net.InputLayer.Count; i++)
			{
				net.InputLayer[i].CopyValues(InputLayer[i]);
			}
			for(int j = 0; j < net.HiddenLayers.Count; j++)
			{
				for(int i = 0; i < net.HiddenLayers[j].Count; i++)
				{
					net.HiddenLayers[j][i].CopyValues(HiddenLayers[j][i]);
				}
			}
			for(int i = 0; i < net.OutputLayer.Count; i++)
			{
				net.OutputLayer[i].CopyValues(OutputLayer[i]);
			}
			return net;
		}
